...
module.exports = function(
    ...
    this.addRoomToColony = function(colony, room) {
        ...
        console.log("add room to colony");
        console.log(JSON.stringify(room));
    ...
    this.processColonyResources = function(colony) {
        ...
        if (
            !colony.lastResourceCheck ||
            colony.lastResourceCheck.level <
                this.game.rooms[colony.mainRoom].controller.level
        ) {
    ...
    this.addSourceOperations = function(colony) {
        ...
        for (const sourceId in colony.sources) {
            let sourceMemory = this.memoryManager.getById(
                OBJECT_TYPE_SOURCE_MEMORY,
                sourceId
            );
            ...
            if (sourceMemory && !sourceMemory.operationId) {
                //create operation
                const source = this.game.getObjectById(sourceId);
                let sourceOperation = this.operationManager.createSourceOperation(
                    source
                );
    ...
    this.processSourceOperations = function(colony) {
        ...
        for (const id in colony.sources) {
            ...
            if (sourceMemory.operationId) {
                let sourceOperation = this.memoryManager.getById(
                    OBJECT_TYPE_SOURCE_OPERATION,
                    sourceMemory.operationId
                );
    ...
    this.areAnyCreepsFree = function(colony, creepType) {
        ...
        return (
            colony.creepQueues[creepType][COLONY_CREEP_QUEUE_FREE].length > 0
        );
    };
};
